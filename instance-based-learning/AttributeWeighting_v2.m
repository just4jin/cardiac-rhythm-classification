...
data_testing = csvread('TrainingData.csv');
count = 1;
...
Final_Results = data_testing;
for k = 1:20
Class_Probability = zeros(size(data_testing,1),3);
for attribute = 2:7
    if attribute == 4
    attribuute =5 ;
    end
Results_attribute = IBL_general_v1(data_training,attribute,k*2);
[ AC1, AC2, AC3 ] = Checker_ILB( Results_attribute, data_training);

Results_attribute = IBL_general_test_v1(data_testing,data_training,attribute,k*2);
...
[ AC1, AC2, AC3 ] = Checker_ILB( Final_Results, data_training );

Accuracy(count) = (AC1(1)*AC1(2) + AC2(1)*AC2(2) + AC3(1)*AC3(2))/(AC1(2) + AC2(2) + AC3(2));
count = count+1;
end