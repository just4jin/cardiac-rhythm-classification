function [Results_attribute] = IBL_general_test_v1(data_testing,data_training,attribute,N)

%Columns 1-PID 2-HR 3-HRV 4-AGE 5-LDS 6-COSEn 7-DFA 8-Class

%Initialize results
Results_attribute = data_testing;
%Find nearest N neighbors for 1 attribute

data_attribute = sortrows(data_training,attribute);
%N needs to be even.s
for test = 1:1;
for Patient = 1:size(data_testing,1)
Difference_old = inf;
Diference_new = 0;
for n = 1:size(data_training,1)
Value = data_testing(Patient,attribute);
...
if lower < 1
    ...
    if upper > size(data_training,1)
        upper = size(data_training,1);
    end
